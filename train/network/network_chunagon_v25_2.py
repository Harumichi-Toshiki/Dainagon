...
INPUT_CHANNELS = 55
CHANNELS = 192
MID_CHANNELS = 48  # 1/4 Nested ボトルネック用
...
class ChunagonNetV3(nn.Module):
    def __init__(self):
        ...
        # --- (1) 拡張レシーバー (Stem) ---
        # 55chの入力を受け止め、いきなりRyncブロックで盤面全体を見渡す
        ...
        # --- (2) 共通幹 (Common Trunk) : 18ブロック (計20) ---
        common_blocks = []
        for i in range(1, 19):
            if i in {3, 8, 13}: # 定期的にRyncで遠距離の利きを確認
                common_blocks.append(RyncNestedBlock(CHANNELS))
            elif i in {5, 10, 15, 18}: # Transformerで全体コンテキストを統合
        ...
        policy_blocks = []
        for i in range(1, 11):
            if i in {5, 10}: # 入り口と出口でTransformer
                policy_blocks.append(HybridTransformerBlock(CHANNELS))
            else:
        ...
        for i in range(1, 11):
            if i == 5: # 中盤で1回だけRync
                value_blocks.append(RyncNestedBlock(CHANNELS))
            else:
        ...
        self.value_head = nn.Sequential(
            ...
            nn.ReLU(inplace=True), 
            nn.Linear(256, 1)
            # 🚨 ここにあった nn.Sigmoid() は削除！（推論時に後乗せ）
        )

        # 🚨 Value Headのバイアスを0初期化（初期局面の評価値バグ対策）
    ...
    def forward(self, x):
        ...
        # 3. Heads (🚨 無傷の h_policy と h_value から計算)
        p = self.policy_head(h_policy).view(-1, 2187)
        v = self.value_head(self.value_conv(h_value).view(-1, 32 * 81))
        
        # 4. MAE (🚨 訓練時のみ、h に対してドロップアウトをかけてから復元させる)
        if self.training:
            h_mae = self.mae_dropout(h) # ここで欠損させる！
